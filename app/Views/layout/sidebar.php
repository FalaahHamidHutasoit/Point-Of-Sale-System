...
<aside class="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <?php if (in_array($role, ['admin', 'kasir'], true)): ?>
            <div class="nav-label">Kasir & Penjualan</div>
            <a class="sidebar-link <?= $path === 'penjualan' ? 'active' : '' ?>" href="<?= base_url('penjualan') ?>"><i class="bi bi-cart-check-fill"></i><span>Penjualan</span></a>
        ...
        <?php elseif ($role === 'manager'): ?>
            <div class="nav-label">Monitoring Penjualan</div>
            <a class="sidebar-link <?= str_starts_with($path, 'penjualan/riwayat') || str_starts_with($path, 'penjualan/sukses') ? 'active' : '' ?>" href="<?= base_url('penjualan/riwayat') ?>"><i class="bi bi-receipt"></i><span>Riwayat Penjualan</span></a>
        <?php endif; ?>

        <?php if ($role === 'admin'): ?>
            <div class="nav-label">Procurement & Gudang</div>
            <a class="sidebar-link <?= $active('pembelian') ?>" href="<?= base_url('pembelian') ?>"><i class="bi bi-clipboard2-check-fill"></i><span>Purchase Order</span></a>
            <a class="sidebar-link <?= $active('penerimaan') ?>" href="<?= base_url('penerimaan') ?>"><i class="bi bi-box-arrow-in-down"></i><span>Penerimaan Barang</span></a>
            ...
            <div class="nav-label">Master Persediaan</div>
        ...
        <?php elseif ($role === 'purchasing'): ?>
            ...
            <a class="sidebar-link <?= $active('pembelian') ?>" href="<?= base_url('pembelian') ?>"><i class="bi bi-clipboard2-check-fill"></i><span>Purchase Order</span></a>
            <a class="sidebar-link <?= $active('penerimaan') ?>" href="<?= base_url('penerimaan') ?>"><i class="bi bi-box-arrow-in-down"></i><span>Histori Penerimaan</span></a>
            <a class="sidebar-link <?= $active('supplier') ?>" href="<?= base_url('supplier') ?>"><i class="bi bi-truck"></i><span>Supplier</span></a>
            <a class="sidebar-link <?= $active('barang') ?>" href="<?= base_url('barang') ?>"><i class="bi bi-box-seam"></i><span>Daftar Barang</span></a>
        <?php elseif ($role === 'gudang'): ?>
            <div class="nav-label">Operasional Gudang</div>
            <a class="sidebar-link <?= $active('penerimaan') ?>" href="<?= base_url('penerimaan') ?>"><i class="bi bi-box-arrow-in-down"></i><span>Penerimaan Barang</span></a>
            <a class="sidebar-link <?= $active('pembelian') ?>" href="<?= base_url('pembelian') ?>"><i class="bi bi-clipboard2-check"></i><span>Purchase Order</span></a>
            <a class="sidebar-link <?= $path === 'stok/mutasi' ? 'active' : '' ?>" href="<?= base_url('stok/mutasi') ?>"><i class="bi bi-arrow-left-right"></i><span>Mutasi Stok</span></a>
            <a class="sidebar-link <?= $active('stok/opname') ?>" href="<?= base_url('stok/opname') ?>"><i class="bi bi-clipboard-check"></i><span>Stock Opname</span></a>
        ...
        <?php if ($role === 'admin'): ?>
            ...
            <a class="sidebar-link <?= $active('management') ?>" href="<?= base_url('management') ?>"><i class="bi bi-compass-fill"></i><span>Pusat Keputusan</span></a>
            ...
            <a class="sidebar-link <?= $active('users') ?>" href="<?= base_url('users') ?>"><i class="bi bi-person-badge-fill"></i><span>Pegawai & Akun</span></a>
        ...
        <?php elseif ($role === 'purchasing'): ?>
            <div class="nav-label">Informasi Procurement</div>
            <a class="sidebar-link <?= $active('laporan/barang') ?>" href="<?= base_url('laporan/barang') ?>"><i class="bi bi-clipboard-data-fill"></i><span>Laporan Persediaan</span></a>
        <?php elseif ($role === 'gudang'): ?>
            <div class="nav-label">Laporan Gudang</div>
            <a class="sidebar-link <?= $active('laporan/barang') ?>" href="<?= base_url('laporan/barang') ?>"><i class="bi bi-clipboard-data-fill"></i><span>Laporan Persediaan</span></a>
        <?php elseif ($role === 'manager'): ?>
            <div class="nav-label">Informasi Manajemen</div>
            <a class="sidebar-link <?= $active('management') ?>" href="<?= base_url('management') ?>"><i class="bi bi-compass-fill"></i><span>Pusat Keputusan</span></a>
            <a class="sidebar-link <?= $active('laporan/penjualan') ?>" href="<?= base_url('laporan/penjualan') ?>"><i class="bi bi-graph-up-arrow"></i><span>Laporan Penjualan</span></a>
            <a class="sidebar-link <?= $active('laporan/barang') ?>" href="<?= base_url('laporan/barang') ?>"><i class="bi bi-clipboard-data-fill"></i><span>Laporan Persediaan</span></a>
            <a class="sidebar-link <?= $active('audit') ?>" href="<?= base_url('audit') ?>"><i class="bi bi-shield-check"></i><span>Audit Aktivitas</span></a>
        ...
        <a class="sidebar-link <?= $active('account/password') ?>" href="<?= base_url('account/password') ?>"><i class="bi bi-key-fill"></i><span>Ganti Password</span></a>
    </nav>

    <div class="sidebar-user">
        <div class="avatar"><?= esc(strtoupper(substr($nama ?: 'U', 0, 1))) ?></div>
        <div class="user-copy"><strong><?= esc($nama ?: 'Pengguna') ?></strong><small><?= esc($roleLabels[$role] ?? ucfirst($role)) ?></small></div>
        <form action="<?= base_url('logout') ?>" method="post" class="logout-form m-0">
            <?= csrf_field() ?>
            <button type="submit" class="logout-icon" title="Logout" aria-label="Logout"><i class="bi bi-box-arrow-right"></i></button>
        </form>
    </div>
</aside>
<style>
...
.sidebar-link{display:flex;align-items:center;gap:11px;padding:10px 12px;margin:2px 0;border-radius:10px;color:#cbd5e1;text-decoration:none;font-size:13px;font-weight:600;transition:.18s}.sidebar-link i{width:20px;font-size:16px}.sidebar-link:hover{background:#1f2937;color:#fff}.sidebar-link.active{background:#2563eb;color:#fff;box-shadow:0 6px 16px rgba(37,99,235,.22)}
