...
<div class="main-content">
    <div class="page-heading">
        ...
        <div class="d-flex gap-2">
            <?php if (session()->get('role') === 'admin'): ?><a class="btn btn-outline-primary" href="<?= base_url('pembelian/tambah') ?>"><i class="bi bi-bag-plus me-1"></i> Restock</a><?php endif; ?>
            <a class="btn btn-primary" href="<?= base_url('penjualan') ?>"><i class="bi bi-plus-lg me-1"></i> Transaksi</a>
        </div>
    </div>
    ...
    <?php if ($stokMenipis): ?><div class="surface-card p-4 mt-3"><div class="d-flex justify-content-between align-items-center mb-3"><div><h6 class="fw-bold mb-1">Perlu Restock</h6><small class="text-muted">Barang dengan stok ≤ 5</small></div><?php if (session()->get('role')==='admin'): ?><a href="<?= base_url('pembelian/tambah') ?>" class="btn btn-sm btn-outline-primary">Buat pembelian</a><?php endif; ?></div><div class="row g-2"><?php foreach ($stokMenipis as $b): ?><div class="col-md-4 col-xl-2"><div class="stock-chip"><strong><?= esc($b['nama_barang']) ?></strong><span><?= esc($b['stok']) ?> <?= esc($b['satuan']) ?> tersisa</span></div></div><?php endforeach; ?></div></div><?php endif; ?>
</div>
<style>
